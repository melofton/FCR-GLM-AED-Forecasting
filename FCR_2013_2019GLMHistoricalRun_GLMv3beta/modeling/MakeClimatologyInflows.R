...
climatology_mean <- dplyr::left_join(datelist, mean_DOY_data) 
climatology_mean1 <- merge(meas_vars, climatology_mean, by="time") %>% 
  select(!(DOY))

write.csv(climatology_mean1, "climatology_mean_weir_inflow.csv",row.names = F)

#now make median climatology
...
climatology_median <- dplyr::left_join(datelist, median_DOY_data) 
